...
class LavadoController extends \BaseController {
	...
	public function index()
	{
		...
		$servicio = $this->servicioRepo->find(1, "");
        ...
        {
            case 2:
		return View::make('lavado.createM', compact('listado_servicios','servicio','rutas'));
            ...
            case 4:
                break;
            default:
				return View::make('lavado.create', compact('listado_servicios','servicio','rutas'));
	...
	public function store()
	{
		...
		$monto= ($kilos>= $data['iptMin'])? $data['iptCosto'] * $kilos : $data['iptCosto'] * $data['iptMin'];
		...
		$data_ticket=array('cliente_id'=>$data['cliente'],'estatus_id'=>$data['estatus_id'],'cupo'=>0,'pago'=>1,'monto'=>$monto);
			...
			if($manager_ticket->isValid()){
				...
				$pedido= $this->pedidoRepo->newPedido();
				$data_pedido=array('ticket_id'=>$ticket_id,'servicio_tipo_id'=>1,'kilos'=>$kilos,'medio_id'=>1,'estatus_id'=>1,'id_colonia_e' => $data['colonia_e'],'id_colonia_r' => $data['colonia_r']);
