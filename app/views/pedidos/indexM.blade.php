    ...
    <table class="table table-hover table-bordered">
        ...
        @foreach ($pedidos->reverse() as $pedido)
            ...
            @endif
                    ...
                    @foreach ($pedido->ticket as $ticket)
                        @foreach ($ticket->cliente as $cliente)
                            ...
                            <?php $cliente=utf8_encode($cliente->nombre);
                                  $id =  utf8_encode($ticket->cliente_id);
                                 $costo= $ticket->monto;
                            ?>
                            {{ HTML::linkAction('ClientesController@show', 'Ver', array(
                                                "0"."/".$id),array('class' => 'btn btn-xs')) }}
                ...
                @if($pedido->estatus_id===1)
                        <a title="Asignar Recolector" data-toggle="modal" href='#modal{{ $modal_id }}'><i class="fa fa-bicycle fa-2x" style="color:#3468F7;"></i></a>
                    ...
                    @if($pedido->estatus_id===3)
                        {{ Form::open(['', '', 'role' => 'form','novalidate', 'id'=>'frme_'.$modal_id]) }}


                            <a title="Recibir en Establecimiento" data-toggle="modal" href='#modal{{ $modal_id }}'><i class="fa fa-home fa-2x" style="color:#49AD53;"></i></a>
                ...
                @if($pedido->estatus_id===4)
                        ...
                        <a title="Asignar Recolector" data-toggle="modal" href='#modal{{ $modal_id }}'><i class="fa fa-bicycle fa-2x" style="color:green;"></i></a>
                        ...
                        {{ Form::close() }}
                    @endif
                    <a title="Asignar Recolector" data-toggle="modal" href='#mdalComentario'><i class="fa fa-comments fa-2x" style="color:#3468F7;"></i></a>
                @if($pedido->estatus_id===1 || $pedido->estatus_id===2 )
               <a href="pedidoOrdenCancelada/{{ $modal_id }}" title="Cancelar Orden">
...
<div class="modal fade" id="mdalComentario" style="display:none" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <input type="area" style="height:300px; width:100%;"/>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
                {{ HTML::link('pedidos', 'Positivo', array('class' => 'btn btn-success')); }}
                {{ HTML::link('pedidos', 'Negativo', array('class' => 'btn btn-danger')); }}
